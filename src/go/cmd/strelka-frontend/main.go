...
func (s *server) IngestFromKafka(bootstrap, topic string) {
    ...
    for {
        ...
        // create Redis keys same as ScanFile
        keyd := fmt.Sprintf("data:%v", raw.Id)
        keye := fmt.Sprintf("event:%v", raw.Id)

        deadline := time.Now().Add(2 * time.Minute) // نفس gRPC deadline

        // write chunk (whole file)
        p := s.coordinator.cli.Pipeline()
        p.RPush(context.Background(), keyd, data)
        p.ExpireAt(context.Background(), keyd, deadline)
        if _, err := p.Exec(context.Background()); err != nil {
            log.Printf("Redis write error: %v", err)
            continue
        }

        log.Printf("Pushed file %s to Redis data:%s", raw.Id, keyd)

        // prepare request metadata
        em := map[string]interface{}{
            "request": request{
                Attributes: &strelka.Attributes{Filename: raw.Filename},
                Client:     raw.Client,
                Id:         raw.Id,
                Source:     raw.Source,
                Time:       raw.Time,
        ...
        requestInfo, _ := json.Marshal(em["request"])

        // Add task to Redis sorted set
        s.coordinator.cli.ZAdd(
            context.Background(),
            "tasks",
            &redis.Z{
                Score:  float64(deadline.Unix()),
        ...
        log.Printf("Added task %s to Redis tasks list", raw.Id)

        // Wait for the result like ScanFile
        go s.waitForResult(raw.Id, keye, em)
    }
}
